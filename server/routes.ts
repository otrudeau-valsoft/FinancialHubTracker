...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Earnings Data Routes

  // Get earnings results for a specific stock
  app.get("/api/earnings/results/:symbol/:region", async (req: Request, res: Response) => {
    try {
      const symbol = req.params.symbol.toUpperCase();
      const region = req.params.region.toUpperCase();
      
      // Query to get most recent earnings results first
      const results = await db.select()
        ...
        .orderBy(desc(earningsResults.reportDate));
      
      return res.json(results);
    } catch (error) {
      console.error("Error fetching earnings results:", error);
      return res.status(500).json({ message: "Failed to fetch earnings results" });
    }
  });

  // Get earnings results for all stocks in a region
  app.get("/api/earnings/results/region/:region", async (req: Request, res: Response) => {
    try {
      const region = req.params.region.toUpperCase();
      ...
      let query = db.select()
        .from(earningsResults)
        .where(eq(earningsResults.region, region));
      
      // Add quarter filter if provided
      if (quarter) {
        query = query.where(eq(earningsResults.quarter, quarter));
      }
      
      // Execute the query
      const results = await query.orderBy(desc(earningsResults.reportDate));
  ...
  // Get upcoming earnings estimates
  app.get("/api/earnings/estimates/upcoming/:region", async (req: Request, res: Response) => {
    try {
      const region = req.params.region.toUpperCase();
      ...
      // Get upcoming earnings ordered by report date
      const estimates = await db.select({
          symbol: earningsEstimates.symbol,
          period: earningsEstimates.period,
          expectedReportDate: earningsEstimates.expectedReportDate,
          consensusEPS: earningsEstimates.consensusEPS,
          company: sql<string>`
            CASE
              WHEN ${earningsEstimates.region} = 'USD' THEN (SELECT company FROM assets_us WHERE symbol = ${earningsEstimates.symbol} LIMIT 1)
              WHEN ${earningsEstimates.region} = 'CAD' THEN (SELECT company FROM assets_cad WHERE symbol = ${earningsEstimates.symbol} LIMIT 1)
              WHEN ${earningsEstimates.region} = 'INTL' THEN (SELECT company FROM assets_intl WHERE symbol = ${earningsEstimates.symbol} LIMIT 1)
            END
          `,
        })
        ...
        .where(
          and(
            ...
            sql`${earningsEstimates.expectedReportDate} >= ${today}`
          )
        )
        .orderBy(earningsEstimates.expectedReportDate)
        .limit(limit);
      
      return res.json(estimates);
    } catch (error) {
      console.error("Error fetching upcoming earnings:", error);
      return res.status(500).json({ message: "Failed to fetch upcoming earnings" });
    }
  });

  // Get analyst recommendations for a specific stock
  app.get("/api/earnings/recommendations/:symbol/:region", async (req: Request, res: Response) => {
    try {
      const symbol = req.params.symbol.toUpperCase();
      const region = req.params.region.toUpperCase();
      ...
      return res.status(500).json({ message: "Failed to fetch analyst recommendations" });
    }
  });

  // Get earnings quarters summary for a region (used in earnings heatmap)
  app.get("/api/earnings/quarters/:region", async (req: Request, res: Response) => {
    try {
      const region = req.params.region.toUpperCase();
      
      // Get quarters sorted by recency
      const quartersQuery = await db.select({
          quarter: earningsResults.quarter,
          count: sql<number>`count(*)`,
        })
        .from(earningsResults)
        .where(eq(earningsResults.region, region))
        .groupBy(earningsResults.quarter)
        .orderBy(desc(earningsResults.quarter));
      
      // Create a simple response with quarters and their stats
      const quarters = [];
      
      for (const q of quartersQuery) {
        // Get stats for this quarter
        const statsQuery = await db.select({
            goodCount: sql<number>`SUM(CASE WHEN earnings_score = 'Good' THEN 1 ELSE 0 END)`,
            okayCount: sql<number>`SUM(CASE WHEN earnings_score = 'Okay' THEN 1 ELSE 0 END)`,
            badCount: sql<number>`SUM(CASE WHEN earnings_score = 'Bad' THEN 1 ELSE 0 END)`,
          ...
          .where(
            and(
              eq(earningsResults.region, region),
              eq(earningsResults.quarter, q.quarter)
            )
          );
        ...
        quarters.push({
          quarter: q.quarter,
          stats: {
            totalReports: q.count,
            earningScore: {
              greatGood: stats.goodCount,
              notSoBad: stats.okayCount,
              ugly: stats.badCount,
            }
          }
        });
      ...
      console.error("Error fetching earnings quarters:", error);
      return res.status(500).json({ message: "Failed to fetch earnings quarters" });
    }
  });

  // Get earnings data for a specific quarter
  app.get("/api/earnings/quarter/:quarter/:region", async (req: Request, res: Response) => {
    try {
      const quarter = req.params.quarter;
      const region = req.params.region.toUpperCase();
      
      // Get detailed data for the quarter
      const earningsData = await db.select({
          symbol: earningsResults.symbol,
          company: sql<string>`
            CASE
              WHEN ${earningsResults.region} = 'USD' THEN (SELECT company FROM assets_us WHERE symbol = ${earningsResults.symbol} LIMIT 1)
              WHEN ${earningsResults.region} = 'CAD' THEN (SELECT company FROM assets_cad WHERE symbol = ${earningsResults.symbol} LIMIT 1)
              WHEN ${earningsResults.region} = 'INTL' THEN (SELECT company FROM assets_intl WHERE symbol = ${earningsResults.symbol} LIMIT 1)
            END
          `,
          epsActual: earningsResults.epsActual,
          epsEstimate: earningsResults.epsEstimate,
          epsSurprisePercent: earningsResults.epsSurprisePercent,
          revenueActual: earningsResults.revenueActual,
          revenueEstimate: earningsResults.revenueEstimate,
          revenueSurprisePercent: earningsResults.revenueSurprisePercent,
          earningsScore: earningsResults.earningsScore,
          marketReaction: earningsResults.marketReaction,
          guidance: earningsResults.guidance,
          stockType: sql<string>`
            CASE
              WHEN ${earningsResults.region} = 'USD' THEN (SELECT stock_type FROM assets_us WHERE symbol = ${earningsResults.symbol} LIMIT 1)
              WHEN ${earningsResults.region} = 'CAD' THEN (SELECT stock_type FROM assets_cad WHERE symbol = ${earningsResults.symbol} LIMIT 1)
              WHEN ${earningsResults.region} = 'INTL' THEN (SELECT stock_type FROM assets_intl WHERE symbol = ${earningsResults.symbol} LIMIT 1)
            END
          `,
          rating: sql<string>`
            CASE
              WHEN ${earningsResults.region} = 'USD' THEN (SELECT stock_rating FROM assets_us WHERE symbol = ${earningsResults.symbol} LIMIT 1)
              WHEN ${earningsResults.region} = 'CAD' THEN (SELECT stock_rating FROM assets_cad WHERE symbol = ${earningsResults.symbol} LIMIT 1)
              WHEN ${earningsResults.region} = 'INTL' THEN (SELECT stock_rating FROM assets_intl WHERE symbol = ${earningsResults.symbol} LIMIT 1)
            END
          `,
          reportDate: earningsResults.reportDate,
        })
        .from(earningsResults)
        .where(
          and(
            eq(earningsResults.region, region),
            eq(earningsResults.quarter, quarter)
          )
        )
        .orderBy(desc(earningsResults.reportDate));
      ...
      return res.json({
        ...
        data: earningsData,
        stats: {
          totalReports: earningsData.length,
          earningScore: {
            greatGood: earningsData.filter(d => d.earningsScore === 'Good').length,
            notSoBad: earningsData.filter(d => d.earningsScore === 'Okay').length,
            ugly: earningsData.filter(d => d.earningsScore === 'Bad').length,
          }
        }
      });
    } catch (error) {
      console.error("Error fetching quarter earnings data:", error);
      return res.status(500).json({ message: "Failed to fetch quarter earnings data" });
    }
  });

  // Import earnings data for a specific stock
  app.post("/api/earnings/import/stock/:symbol/:region", async (req: Request, res: Response) => {
    try {
      const symbol = req.params.symbol.toUpperCase();
      const region = req.params.region.toUpperCase();
      
      // Run the import as a background process
      const importProcess = spawn('npx', ['tsx', 'scripts/import-earnings-data.ts', 'stock', symbol]);
      
      importProcess.stdout.on('data', (data) => {
        console.log(`Earnings import stdout: ${data}`);
      });
      
      importProcess.stderr.on('data', (data) => {
        console.error(`Earnings import stderr: ${data}`);
      });
      
      importProcess.on('close', (code) => {
        console.log(`Earnings import process exited with code ${code}`);
      });
      
      return res.json({ 
        message: `Started earnings data import for ${symbol} (${region})`, 
        status: 'In Progress' 
      });
    } catch (error) {
      console.error("Error importing earnings data:", error);
      return res.status(500).json({ message: "Failed to import earnings data" });
    }
  });

  // Import earnings data for a region
  app.post("/api/earnings/import/region/:region", async (req: Request, res: Response) => {
    try {
      const region = req.params.region.toUpperCase();
      
      // Run the import as a background process
      const importProcess = spawn('npx', ['tsx', 'scripts/import-earnings-data.ts', 'region', region]);
      ...
      return res.json({ 
        message: `Started earnings data import for ${region} region`, 
        status: 'In Progress' 
      });
    } catch (error) {
      console.error("Error importing earnings data:", error);
  ...
  app.post("/api/earnings/import/all", async (req: Request, res: Response) => {
    try {
      // Run the import as a background process
      const importProcess = spawn('npx', ['tsx', 'scripts/import-earnings-data.ts', 'all']);
      ...
      return res.json({ 
        message: `Started earnings data import for all portfolios`, 
        status: 'In Progress' 
      });
    } catch (error) {
      console.error("Error importing earnings data:", error);
